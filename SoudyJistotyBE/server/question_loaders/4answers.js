...
rearrangedQuestions.forEach((question, index) => {
  ...
  const baseOptionId = index * 4 + 1 + 80 // Adjust based on actual ID sequencing logic

  // INSERT statement for the Question, linking to the QOptions
  sqlStatements += `INSERT INTO Question (questionType, description, option1, option2, option3, option4, type, variant) VALUES ('${question.questionField.replace(
    "'",
    "''"
  )}', '${question.questionDescription.replace("'", "''")}', ${baseOptionId}, ${
    baseOptionId + 1
  }, ${baseOptionId + 2}, ${baseOptionId + 3}, 'alltext', 'A');\n\n`
})

// Write the SQL statements to a file
fs.writeFile('inserts_with_four_options.sql', sqlStatements, (err) => {
  if (err) throw err
  console.log(
    'SQL insert statements have been written to inserts_with_four_options.sql'
  )
})
